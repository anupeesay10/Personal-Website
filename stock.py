...
# API endpoint: set via environment variable for Render
API_URL = os.environ.get("API_URL", "http://localhost:5000/api")  # Replace with Render service URL in env
...
def load_stock_data(n_clicks, init_trigger, ticker, start_date, end_date):
    ...
    if trigger_id == 'initialization-div':
        ...
        if result:
            ...
            message = f"Data for {DEFAULT_TICKER} retrieved from {source}!"
    ...
    try:
        ...
        message = f"Data for {ticker.upper()} loaded from {source}!"
...
@app.callback(Output('select-year', 'disabled'), Input('stat-select', 'value'))
...
def update_output(json_data, stat_type, year, ticker):
    ...
    # [The rest of your chart creation code can remain unchanged here]
    # Return the list of dcc.Graph objects as before
    return []
